...
class WebInterfaceTestCase(unittest.TestCase):
    ...
    @unittest.mock.patch("app.extract_text_with_layout_and_columns")
    def test_convert_via_url_pdf_success(self, mock_extract, mock_pdfplumber_open, mock_get):
        mock_response = unittest.mock.Mock()
        mock_response.status_code = 200
        ...
        mock_extract.return_value = "Controle de PDF por URL com sucesso!"
        ...
        self.assertEqual(response.status_code, 200)
        self.assertTrue(response.json["success"])
        self.assertIn("Controle de PDF por URL com sucesso!", response.json["content"])
        self.assertEqual(response.json["filename"], "documento.pdf")

    @unittest.mock.patch("requests.get")
    @unittest.mock.patch("pdfplumber.open")
    @unittest.mock.patch("app.extract_text_with_layout_and_columns")
    def test_convert_via_url_no_extension_pdf(self, mock_extract, mock_pdfplumber_open, mock_get):
        mock_response = unittest.mock.Mock()
        mock_response.status_code = 200
        mock_response.content = b"%PDF-1.4 dummy content"
        mock_response.headers = {"Content-Type": "application/pdf"}
        ...
        mock_pdf.pages = [mock_page]
        mock_pdfplumber_open.return_value.__enter__.return_value = mock_pdf
        mock_extract.return_value = "PDF sem extensao na URL!"

        response = self.client.post(
            "/api/convert",
            data={
                "url": "https://exemplo.com/download?id=999",
                "option": "standard",
            }
        )
        ...
        self.assertEqual(response.status_code, 200)
        self.assertTrue(response.json["success"])
        self.assertIn("PDF sem extensao na URL!", response.json["content"])
        self.assertEqual(response.json["filename"], "documento.pdf")

    @unittest.mock.patch("requests.get")
    def test_convert_via_url_ssl_fallback(self, mock_get):
        import requests.exceptions
        
        # First call raises SSLError, second call succeeds with verify=False
        mock_response = unittest.mock.Mock()
        mock_response.status_code = 200
        mock_response.content = b"<html><body><h1>SSL Fallback</h1></body></html>"
        ...
        response = self.client.post(
            "/api/convert",
            data={
                "url": "https://exemplo-com-ssl-erro.com/doc.html",
                "option": "standard",
            }
        )
        ...
        self.assertEqual(response.status_code, 200)
        self.assertTrue(response.json["success"])
        self.assertIn("SSL Fallback", response.json["content"])
        self.assertEqual(mock_get.call_count, 2)
        
        # Check first call had verify=True or no verify keyword
        first_args, first_kwargs = mock_get.call_args_list[0]
        self.assertNotIn("verify", first_kwargs)
        
        # Check second call had verify=False
        second_args, second_kwargs = mock_get.call_args_list[1]
        self.assertEqual(second_kwargs.get("verify"), False)
